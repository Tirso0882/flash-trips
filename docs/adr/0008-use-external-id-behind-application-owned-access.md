...
Entra becomes the final implementation commitment only after a disposable integration spike verifies Google and email-code sign-in, the Next.js session boundary, FastAPI API-audience token validation, internal Invitation enforcement, safe account linking, immediate suspension, Operator bootstrap, local development, and redacted telemetry without preview invitations or provider groups. Guest transition is not part of this pilot spike because Guest Session and Guest Intake Transfer are deferred. Auth0 is selected instead if a correctness or security requirement needs an unstable feature or disproportionate workaround.
...
The invitation-only release requires authentication before all Trip Intake. Guest Session, Guest General, and Guest Intake Transfer are deferred together. If a Guest surface is later introduced, it remains browser-bound and rate-limited; permits neither personalised planning nor inventory searches, Trip ownership, Plan Revisions, Approvals, cross-device continuation, nor handbook access; and cannot make guest conversation content authoritative automatically.
