...
- **Approval and execution progress:** why an Approval Gate was inserted; whether the Approval Request is a pre-commit planning decision over an uncommitted proposed-result Fingerprint, accepted base Plan Revision, exact Evidence, and policy, or a post-commit Handbook delivery decision over a committed Plan Revision and exact eligible snapshot inputs; the exact binding and lifecycle; decision, expiry, or supersession; and resume-context comparisons. Execution Checkpoints expose only identity, schema and version, integrity reference, progress position, and resume decision, never raw private LangGraph state.
