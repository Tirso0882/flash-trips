...
`https://localhost:3000/planner`, the Operator shell is at
`https://localhost:3000/operator`, and the BFF status route is at
`https://localhost:3000/api/status`.

Next.js serves HTTPS locally because the OIDC redirect URI registered in Entra
External ID is `https://localhost:3000/api/auth/callback`, and a provider only
redirects to the exact registered URI. Next reuses a certificate at
`apps/web/certificates/localhost.pem`, which is Git-ignored, and generates one
with mkcert when it is missing. That generation runs `mkcert -install`, which
needs sudo and fails on a machine whose sudo demands an interactive prompt. If
it does, issue the certificate yourself once, without touching the trust store:

```sh
cd apps/web/certificates
"$HOME/Library/Caches/mkcert/mkcert-v1.4.4-darwin-arm64" \
  -key-file localhost-key.pem -cert-file localhost.pem localhost 127.0.0.1 ::1
```

The issuing CA then stays out of the system trust store, so the browser warns
once per host until you accept the exception.
...
Feature work is sequenced by ADR 0021: a thin, permanent, browser-asserted path
through the whole product lands first, then each station is deepened. Read that
